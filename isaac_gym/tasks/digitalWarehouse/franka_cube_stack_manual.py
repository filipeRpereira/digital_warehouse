...
table_dims = gymapi.Vec3(0.0, 0.0, 1.0)
...
box_size_B = 0.045
asset_options = gymapi.AssetOptions()
box_asset_B = gym.create_box(sim, box_size, box_size, 0.3, asset_options)
...
franka_pose.p = gymapi.Vec3(-0.45, 0.0, 0.8 + table_thickness / 2)
...
for i in range(num_envs):
    ...
    cube_color_B = gymapi.Vec3(0.6, 0.1, 0.0)
    gym.set_rigid_body_color(env, box_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, cube_color_B)

    # add box B
    box_pose_B.p.x = 0.2
    box_pose_B.p.y = -0.3
    box_pose_B.p.z = table_dims.z + 0.5 * box_size_B
    box_pose_B.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)
    box_B_handle = gym.create_actor(env, box_asset_B, box_pose_B, "box_B", i, 0)
    cube_color_B = gymapi.Vec3(0.0, 0.4, 0.1)
    gym.set_rigid_body_color(env, box_B_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, cube_color_B)
