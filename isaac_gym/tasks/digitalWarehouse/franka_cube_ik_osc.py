...
# create table asset
table_dims = gymapi.Vec3(1.2, 1.2, 0.05)
...
box_A_size = 0.045
asset_options = gymapi.AssetOptions()
box_A_asset = gym.create_box(sim, box_A_size, box_A_size, box_A_size, asset_options)

# create box B asset
box_B_size = 0.045
asset_options = gymapi.AssetOptions()
box_B_asset = gym.create_box(sim, box_B_size, box_B_size, 0.3, asset_options)
...
franka_pose.p = gymapi.Vec3(-0.45, 0, 0.05)

table_pose = gymapi.Transform()
table_pose.p = gymapi.Vec3(0.0, 0.0, 0.5 * table_dims.z)

box_A_pose = gymapi.Transform()
box_B_pose = gymapi.Transform()
...
for i in range(num_envs):
    ...
    box_A_pose.p.x = 0.1
    box_A_pose.p.y = 0.1
    box_A_pose.p.z = table_dims.z + 0.5 * box_A_size
    box_A_pose.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)
    box_A_handle = gym.create_actor(env, box_A_asset, box_A_pose, "box_A", i, 0)
    box_A_color = gymapi.Vec3(0.6, 0.1, 0.0)
    gym.set_rigid_body_color(env, box_A_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, box_A_color)

    # get global index of box in rigid body state tensor
    box_A_idx = gym.get_actor_rigid_body_index(env, box_A_handle, 0, gymapi.DOMAIN_SIM)
    box_A_idxs.append(box_A_idx)

    # add box B
    box_B_pose.p.x = 0.2
    box_B_pose.p.y = -0.3
    box_B_pose.p.z = table_dims.z + 0.5 * 0.3
    box_B_pose.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)
    box_B_handle = gym.create_actor(env, box_B_asset, box_B_pose, "box_B", i, 0)
    box_B_color = gymapi.Vec3(0.0, 0.4, 0.1)
    gym.set_rigid_body_color(env, box_B_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, box_B_color)

    # get global index of box in rigid body state tensor
    box_B_idx = gym.get_actor_rigid_body_index(env, box_B_handle, 0, gymapi.DOMAIN_SIM)
    box_B_idxs.append(box_B_idx)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    box_A_pos = rb_states[box_A_idxs, :3]
    box_A_rot = rb_states[box_A_idxs, 3:7]

    box_B_pos = rb_states[box_B_idxs, :3]
    box_B_rot = rb_states[box_B_idxs, 3:7]

    hand_pos = rb_states[hand_idxs, :3]
    hand_rot = rb_states[hand_idxs, 3:7]
    hand_vel = rb_states[hand_idxs, 7:]

    # distance between hand and box A
    to_box = box_A_pos - hand_pos
    box_dist = torch.norm(to_box, dim=-1).unsqueeze(-1)
    box_dir = to_box / box_dist
    box_dot = box_dir @ down_dir.view(3, 1)

    # distance between box A and box B
    box_A_to_box_B = box_B_pos - box_A_pos
    ...
    gripped = (gripper_sep < 0.045) & (box_dist < grasp_offset + 0.5 * box_A_size)

    yaw_q = cube_grasping_yaw(box_A_rot, corners)
    ...
    return_to_start = gripped.squeeze(-1).unsqueeze(-1)
    ...
    grasp_pos = box_A_pos.clone()
    grasp_pos[:, 2] = torch.where(above_box, box_A_pos[:, 2] + grasp_offset, box_A_pos[:, 2] + grasp_offset * 2.5)
    ...
    #hand_restart = hand_restart | (box_A_pos[:, 2] > 0.7)
