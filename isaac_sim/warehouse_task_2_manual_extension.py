...
from omni.isaac.examples.user_examples import WarehouseTask2Manual
...
class WarehouseTask2ManualExtension(BaseSampleExtension):
    def on_startup(self, ext_id: str):
        super().on_startup(ext_id)
        super().start_extension(
            menu_name="WareHouse",
            submenu_name="Tasks 2",
            name="Manual Programing",
            title="Warehouse project",
            doc_link="",
            overview="An warehouse project.",
            file_path=os.path.abspath(__file__),
            sample=WarehouseTask2Manual(),
        )
    ...
    def _on_stacking_button_event(self):
        ...
        self.task_ui_elements["Task 2"].enabled = False
        return

    def post_reset_button_event(self):
        self.task_ui_elements["Task 2"].enabled = True
        return
    ...
    def build_task_controls_ui(self, frame):
        with frame:
            with ui.VStack(spacing=5):
                ...
                self.task_ui_elements["Task 2"] = btn_builder(**dict)
                self.task_ui_elements["Task 2"].enabled = False
